...
def test_profile_extraction_validation_reports_missing_required_data():
    extraction = ProfileExtraction(name="آية", age=24, province=None, gender="female")
...
def test_profile_extraction_does_not_invent_contact_data():
    extraction = ProfileExtraction(name="آية", age=24, province="دمشق", gender="female")
...
def test_search_filter_extraction_accepts_age_range():
    parsed = SearchFilterExtraction(age_min=22, age_max=27, province="دمشق")
...
def test_basic_profile_fallback_extracts_arabic_digits_without_inventing_missing_data():
    ...
    assert parsed.province == "ريف دمشق"
    assert parsed.city == "جرمانا"
...
def test_ai_private_contacts_are_replaced_by_deterministic_values():
    ai_result = ProfileExtraction(
        phone="0000000000", telegram_username="invented"
    )
    deterministic = ProfileExtraction(
        phone="0911223344", telegram_username="real_user"
    )
