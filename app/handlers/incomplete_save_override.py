"""Hardened save path for incomplete Admin V2 profiles."""
...
def _canonical_draft(draft: ProfileDraft) -> ProfileDraft:
...
async def _save_add(update: Any, context: Any) -> int:
    ...
    try:
        ...
        with admin_v2._session(context) as session:
            # Do not trust the preview-time request number. The database allocates
            # the profile id at save time, and the repository derives the request
            # number from that id, avoiding stale/concurrent preview collisions.
            profile = ProfileRepository(session).create(draft, request_number=None)
        ...
        try:
            ...
            # The profile is already committed; a Telegram edit failure must not
            # turn a successful save into the generic application error.
            logger.warning("Saved profile %s but Telegram edit failed", number, exc_info=True)
            try:
                await update.effective_message.reply_text(
                    message_text, reply_markup=admin_v2._dashboard_keyboard()
                )
            except Exception:
                logger.warning("Fallback Telegram reply failed for saved profile %s", number, exc_info=True)
        return admin_v2.END
        ...
        await update.callback_query.edit_message_text(
            ...
            "ما تم اعتماد أي جزء من العملية. رجّع للإضافة وجرّب مرة ثانية.",
            reply_markup=admin_v2._dashboard_keyboard(),
        )
        return admin_v2.END
    except (SQLAlchemyError, ValueError, TypeError):
        logger.exception("Database/value error while saving incomplete profile")
        await update.callback_query.edit_message_text(
