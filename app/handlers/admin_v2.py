...
async def _create_backup(update: Any, context: Any) -> int:
    ...
    await update.callback_query.message.reply_document(InputFile(payload, filename=f"naseb-backup-{backup_id}.json"), caption=f"💾 نسخة احتياطية #{backup_id}")
...
async def _download_last_backup(update: Any, context: Any) -> int:
    ...
    await update.callback_query.message.reply_document(InputFile(row.snapshot_json.encode("utf-8"), filename=f"naseb-backup-{row.id}.json"), caption=f"💾 النسخة #{row.id}")
