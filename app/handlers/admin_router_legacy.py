...
async def _show_reservations_plus(update: Any, context: Any, page: int) -> int:
    ...
    if not rows:
        await update.callback_query.edit_message_text("🔒 ما في عروض محجوزة حالياً.", reply_markup=admin_v2._dashboard_keyboard())
        return END
    text_body = "🔒 الحجوزات\n\n"; buttons=[]
...
async def admin_callback(update: Any, context: Any) -> int:
    ...
    if parsed_extension is not None:
        ...
        with _session(context) as session:
            profile=ProfileRepository(session).get(number)
            if not profile or profile.status != "reserved":
                await update.callback_query.edit_message_text("❌ الإعلان مو محجوز حالياً.", reply_markup=admin_v2._dashboard_keyboard())
                return END
            meta=get_profile_meta(session,profile.id,True)
...
async def admin_text(update: Any, context: Any) -> int:
    ...
    if flow == "danger_selected_confirm":
        ...
        context.user_data.clear(); await update.effective_message.reply_text(f"✅ تم حذف {len(numbers)} إعلان نهائياً بعد إنشاء نسخة احتياطية.",reply_markup=admin_v2._dashboard_keyboard()); return END
    if flow == "danger_all":
        ...
        context.user_data.clear(); await update.effective_message.reply_text(f"✅ تم حذف {count} إعلان نهائياً بعد إنشاء نسخة احتياطية.",reply_markup=admin_v2._dashboard_keyboard()); return END
