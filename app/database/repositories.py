...
class ProfileRepository:
    ...
    def search(self, filters: ProfileFilters, include_inactive: bool = False) -> list[Profile]:
        ...
        if filters.province:
            stmt = stmt.where(Profile.province.ilike(f"%{filters.province.strip()}%"))
        if filters.city:
            stmt = stmt.where(Profile.city.ilike(f"%{filters.city.strip()}%"))
...
def export_all_data(session: Session) -> dict:
    ...
    orders_stmt = select(Order).order_by(Order.id)
    orders = []
    for order in session.scalars(orders_stmt).all():
        orders.append({
