...
def format_marriage_post(profile: dict) -> str:
    ...
    if profile.get("occupation"):
        lines.extend(["", f"العمل: {profile['occupation']} 💼"])
    if profile.get("education"):
        lines.extend(["", f"المستوى التعليمي: {profile['education']} 📚"])
    if profile.get("marital_status"):
        lines.extend(["", f"الحالة الاجتماعية: {profile['marital_status']} 💍"])
    ...
    lines.extend([
        ...
        "",
        _partner_requirements_text(profile.get("partner_requirements")),
        "",
